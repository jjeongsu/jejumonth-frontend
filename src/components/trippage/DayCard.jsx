...
export default function DayCard({number,date}) {
  ...
  const handleClick = () => {
    navigate(`/plan/add-plan?trip_id=${tripId}?date=${date}`);
  }
  ...
  return (
    <div className="w-371 h-66 ml-26 relative">
      ...
      <div className="w-80 left-81 top-0 absolute text-gray-6 text-xs font-semibold">
        {date}
      </div>
      ...
      <button onClick={handleClick} className="w-23 h-20 left-189 top-45 absolute text-center text-primary-0 text-13 font-medium">
        추가
      </button>
    </div>
